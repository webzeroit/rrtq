...
class Archiviopubblicazioni extends MY_Controller_Admin
{
    ...
    public function index()
    {
        ...
        $this->output->set_title("Archivio Pubblicazioni");
        $this->load->view('qualificazione/archivio');
    }
    ...
    public function get_datatables_json()
    {
        ...
        $this->load->model('archivio_pubblicazioni_model', 'archivio');
        $output = $this->archivio->datatables_archivio_pubblicazioni();
        $this->_render_text($output);
    }

    public function genera_sp($id_pubblicazione = NULL)
    {

        $this->load->model('archivio_pubblicazioni_model', 'archivio');
        $file_qualificazione = $this->archivio->select_storico_serialized($id_pubblicazione);
        ...
        $id_profilo = $file_qualificazione["profilo"]["id_profilo"];
        $des_stato_profilo = "Archiviato";
        //PROCESSO
    ...
    public function genera_sf($id_pubblicazione = NULL)
    {
        $this->load->model('archivio_pubblicazioni_model', 'archivio');
        $file_qualificazione = $this->archivio->select_storico_serialized($id_pubblicazione);
        ...
        {
            $id_standard_formativo = $sf["id_standard_formativo"];
            $standard_formativo = $sf;
            $this->download_sf($profilo,$standard_formativo,$id_standard_formativo);
  
        }
        
    }
        
    private function download_sf($profilo, $standard_formativo, $id_standard_formativo)
    {
        ...
        // imposta metadata documento
        $pdf->document_id = $document_id;
        $pdf->document_status = "Archiviato";
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('Regione Campania');
